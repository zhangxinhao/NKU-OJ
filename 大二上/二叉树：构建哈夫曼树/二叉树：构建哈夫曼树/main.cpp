...
ofstream output("/Users/apple/Documents/HuffmanCode.txt");
#define MAX   17853
unsigned int ac = 0,bc = 0;
//Huffman 树的结构
typedef struct
{
    //单词
...
void createHuffmanTree(HuffmanTree *huffmanTree, int n)
{
    ...
    *huffmanTree = new Node[m + 1];
    ...
    for(i = 1; i <= n; i++)
    {
        input>>(*huffmanTree)[i].str;
        input>>(*huffmanTree)[i].weight;
...
void creatHuffmanCode(HuffmanTree *huffmanTree, HuffmanCode *huffmanCode, int n)
{
    ...
    for(i = 1; i <= n; i++)
    {
        output << (*huffmanTree)[i].str << " " << huffmanCode[i]<<endl;
    }
    //计算压缩比例
    for(i = 1; i <= n; i++)
    {
        ac+=(*huffmanTree)[i].weight * (*huffmanTree)[i].str.length() * 8;
        
    }
    for(i = 1; i <= n; i++)
    {
        bc+=(*huffmanTree)[i].weight * strlen(huffmanCode[i]);
    }
